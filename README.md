...
```cpp
std::vector< hiberlite::bean_ptr<MyClass> > v=db.getBeans<MyClass>(std::string where, std::string orderBy);
```

will return a vector with all objects that matches the where in the database.
